...
create index if not exists idx_documents_embedding_hnsw on documents using hnsw (embedding vector_ip_ops)
...
-- ----------------------------------------------------------------------------
ALTER TABLE public.crm_clients DROP COLUMN IF EXISTS bmi_data;
...
COMMENT ON COLUMN public.crm_clients.conversation_stage IS 'Bot sales funnel: first_contact -> browsing -> product_viewed -> order_placed -> purchased. "support" = complaint/human handoff (off-funnel).';
...
GRANT
EXECUTE ON FUNCTION public.update_client_stage (TEXT, UUID, TEXT) TO service_role;

GRANT
EXECUTE ON FUNCTION public.update_client_stage (TEXT, UUID, TEXT) TO authenticated;
  ...
  IF p_category IS NOT NULL AND TRIM(p_category) != '' THEN
    ...
    SET 
      ...
      client_type = CASE 
        ...
        ELSE client_type -- يحافظ على حالة المشتري customer إذا لم تُطلب حالة صريحة
      END,
      updated_at = NOW()
